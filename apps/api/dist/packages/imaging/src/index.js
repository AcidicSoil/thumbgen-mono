...
export async function extractPalette(buf) {
...
function gradientSVG(size, c1, c2) {
...
function titlePlateSVG(w, h, title, subtitle) {
    const margin = 28;
...
async function resizeToFit(buf, box) {
    const img = sharp(buf);
...
export async function generateThumbnail(images, opts = {}) {
    if (!images.length)
